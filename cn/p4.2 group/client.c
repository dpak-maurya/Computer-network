...
void *reader(void * s)
{
	...
	while(1)
	{
		int n=read(fd,buff,sizeof(buff));//pipe
		if(n==0){}
		else
		{
			buff[n]='\0';
			write(1,buff,strlen(buff)+1);//term
		}
	}
}
void * writter(void *s)
{
	...
	while(1)
	{
		int n=read(0,buff,sizeof(buff));//term
		if(n==0){}
		else
		{
			buff[n]='\0';
			...
			strncat(buff3,&ck,1);
			char gnbuff='0'+gn;
			strncat(buff3,&gnbuff,1);
			strncat(buff3,&ck,1);
			strcat(buff3,buff);
			//printf("%s",buff);
			//printf("%s %c %s %s \n",buff2,'/',buff,buff3);
			write(fds,buff3,strlen(buff3)+1);//pipe
