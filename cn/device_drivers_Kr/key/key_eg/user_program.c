...
int main()
{
	int fd=open("/dev/led_driver",O_RDWR); // opens the device driver which has been loaded
	if(fd<0)
	perror("error");
	printf("%d\n",fd);
	char buf[100];
	...
	int c=write(fd,buf,strlen(buf)); // writes the time this changes the system time
